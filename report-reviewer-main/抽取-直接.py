...
注意区分原文和所给实体名的细微差异，如测风塔海拔高度与项目海拔高度不一致。"""
    ...
    instruction = f"""文本中{enter_type}的具体实体信息或数值是？{response1}\n\n在上述回答基础上，判断文本中是否包含“{enter_type}”的具体实体信息，实体信息应与实体类型直接对应而不是相关，如项目海拔高度和测风塔海拔高度，他们虽然都讲述了海拔高度，但主体不同，请你排除这些冗余信息的干扰,输出True或False。"""
...
5.若文本中涉及多个方案对比，你仅需要提取本项目最终选定的属性值。


问题：文本中是否直接提及【{enter_type}】的实体信息？回答：{response1}
